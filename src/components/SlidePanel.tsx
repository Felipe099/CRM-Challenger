...
export function SlidePanel({
    ...
    const handleSave = async () => {
        ...
        if (leadData) {
            ...
            try {
                await onSave(leadData.id, editedData);
                setIsEditing(false);
            } catch (error) {
                const errorMessage =
    ...
    const handleConvertLead = async () => {
        ...
        try {
            await simulateApiCall(1000, 0.08);
            ...
            if (clients.some((client: Lead) => client.id === leadData.id)) {
                setConversionError('Lead already converted');
                return;
            }
            ...
            const storedLeads = safeLocalStorage.getItem('contacts');
            if (storedLeads) {
                            ...
                            {isEditing ? (
                                ...
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        disabled={
                                            isAlreadyConverted || isConverting
                                        }
                                        className={`flex-1 flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-white font-semibold transition-all duration-200 ${
                                            isAlreadyConverted || isConverting
                                                ? 'bg-gray-400 cursor-not-allowed'
